...
class BookSynopsisDelegate(
    ...
    private var currentStartIndexMax: Int = GameConstants.BOOK_SYNOPSIS_START_INDEX_MAX_INIT
        ...
        scope.coroutineScope.launch {
            scope.updateState {
                copy(
                    ...
                    modeState = GameContract.ModeState.BookSynopsis()
                )
            }
            ...
            scope.updateState {
                copy(
                    ...
                    modeState = GameContract.ModeState.BookSynopsis(coins = initialCoins)
                )
            }
    ...
    override fun onAnswerSubmitted(answer: String) {
        ...
        val bookState = state.bookSynopsisState ?: return

        val optionId = (currentQuestion.content as? QuestionContent.BookSynopsis)
            ?.options?.find { it.bookTitle.equals(answer.trim(), ignoreCase = true) }?.bookId ?: ""

        if (isCorrect) {
            val newStreak = state.currentStreak + 1
            val newBestStreak = maxOf(bookState.bestSessionStreak, newStreak)
            val scoreGain = GameConstants.BOOK_SYNOPSIS_BASE_POINTS + GameConstants.BOOK_SYNOPSIS_STREAK_BONUS * newStreak
            val newScore = bookState.currentScore + scoreGain
            val newCoins = bookState.coins + GameConstants.BOOK_SYNOPSIS_COINS_PER_CORRECT * coinMultiplier
            ...
            // Expand obscurity after enough correct answers
            if (newCorrect % GameConstants.BOOK_SYNOPSIS_START_INDEX_EXPAND_STEP == 0) {
                currentStartIndexMax += GameConstants.BOOK_SYNOPSIS_START_INDEX_EXPAND_AMOUNT
            }
            ...
            scope.coroutineScope.launch {
                try {
                    ...
                    updateUserStatsUseCase(
                        stats.copy(
                            coins = stats.coins + GameConstants.BOOK_SYNOPSIS_COINS_PER_CORRECT,
            ...
            scope.updateState {
                copy(
                    ...
                    currentStreak = 0,
                    searchResults = emptyList(),
                    modeState = bookState.copy(selectedOptionId = optionId)
                )
            }
        }
    }
    ...
    override fun onUseHint() {
        ...
        if (bookState.coins < GameConstants.BOOK_SYNOPSIS_HINT_COST) {
            scope.updateState {
        ...
        val content = currentQuestion.content as? QuestionContent.BookSynopsis ?: return
        val wrongOptions = content.options.filter { it.bookTitle != currentQuestion.correctAnswer }
        val toRemove = wrongOptions.shuffled().take(2).map { it.bookId }.toSet()

        val newCoins = bookState.coins - GameConstants.BOOK_SYNOPSIS_HINT_COST
        scope.updateState {
            copy(
                modeState = bookState.copy(
                    coins = newCoins,
    ...
    override fun onRetryGame() {
        ...
        seenItemIds.clear()
        questionQueue.clear()
        currentStartIndexMax = GameConstants.BOOK_SYNOPSIS_START_INDEX_MAX_INIT
    ...
    override fun onExitGame() {
        scope.coroutineScope.launch {
            val bookState = scope.currentState().bookSynopsisState
    ...
    private suspend fun fetchBatchAndStart() {
        fetchBookSynopsisBatchUseCase(
            categoryType,
            GameConstants.BOOK_SYNOPSIS_BATCH_SIZE,
    ...
    private fun presentNextQuestion() {
        if (questionQueue.isEmpty()) {
            scope.updateState {
                val bs = bookSynopsisState ?: return@updateState this
        ...
        scope.updateState {
            val bs = bookSynopsisState ?: GameContract.ModeState.BookSynopsis()
            copy(
    ...
    private fun prefetchIfNeeded() {
        if (questionQueue.size <= GameConstants.BOOK_SYNOPSIS_PREFETCH_THRESHOLD && prefetchJob?.isActive != true) {
